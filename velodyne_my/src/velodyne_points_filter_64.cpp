...
#define  PI M_PI
ros::Publisher pub;
pcl::PointCloud<pcl::PointXYZI>::Ptr laserCloudScans[64];
...
//ROS_INFO ("laserCloudIn->points[cloudSize - 33].x: %f  , laserCloudIn->points[cloudSize - 33].y: %f ", laserCloudIn->points[cloudSize - 33].x,laserCloudIn->points[cloudSize - 33].y);
 ...
 //ROS_ERROR ("XstartOri: %f  , XendOri: %f ", startOri,endOri);
   ...
   for (int i = 0; i < cloudSize; i++) {
    ...
    const float angleStep = angleSpan / (64 - 1);
    float angleID = (angle - angleLowerBoundDeg) / angleStep;
    
    scanID = int(angleID + 0.5f);

 if (scanID > (64 - 1) || scanID < 0) { // drop the points with invalid scanIDs
 ...
 for (int i = 0,j = 0; (i < 64) && (j < p_obstacles_detection_scans_); i=i+p_obstacles_detection_interval_,j=j+1) {
...
{
  	// Initialize ROS
  	ros::init (argc, argv, "velodyne_points_filter_64");
	ros::NodeHandle nh;
	...
	pub = nh.advertise<sensor_msgs::PointCloud2> ("/velodyne_points_filtered_64", 2);
	// Spin
	 for (int i = 0; i < 64; i++) {
